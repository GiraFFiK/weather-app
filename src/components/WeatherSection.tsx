...
export default function WeatherSection({
  ...
  const getWeatherIcon = (code: string | number): string => {
    ...
    return weatherIcons[codeStr];
  };

  function formatTemp(tempC: number, unit: string): number {
    if (unit === "fahrenheit") {
  ...
  function formatWind(speedMs: number, unit: string): number {
    if (unit === "mp/h") {
  ...
  function formatPrecipitation(mm: number, unit: string): number {
    if (unit === "in") {
  ...
  let weekDay: string = new Intl.DateTimeFormat("en-EN", {
    weekday: "long",
  }).format(currentDate);
  let month: string = new Intl.DateTimeFormat("en-EN", {
    month: "short",
  }).format(currentDate);
      ...
      <div className="weather__inner">
        <div className="weather__main">
          <div className="weather__main__info">
            <div className="weather__main__info__citydata">
              {city + ", " + country}
              <br />
              <span className="weather__data">
            ...
            <div className="weather__main__info__temperature">
              <img
                ...
                src={wcode !== undefined ? getWeatherIcon(wcode): ""}
                alt=""
              />
              {temp
                ? formatTemp(Math.round(temp), weatherUnits.temperature)
                : "—"}
              °
            </div>
          </div>
          <div className="weather__main__forecast">
            <ul className="forecast__list">
              <li className="forecast__item">
                ...
                <div className="item__feel__value">
                  {feelsLike
                    ? formatTemp(feelsLike, weatherUnits.temperature)
                    : "—"}
                  °
                </div>
              </li>
              ...
              <li className="forecast__item">
                ...
                <div className="item__wind__value">
                  {wind !== undefined ?  wind >= 0
                    ? formatWind(Math.round(wind), weatherUnits.wind)
                    : "—": ""}{" "}
                  {weatherUnits.wind}
                </div>
              </li>
              <li className="forecast__item">
                ...
                <div className="item__precipitation__value">
                  {prec !== undefined ?  prec >= 0
                    ? formatPrecipitation(prec, weatherUnits.precipitation)
                    : "—": ""}{" "}
          ...
          <ul className="weather__daily__forecast">
            {daily.map((day, index: number) => (
              <li key={index} className="daily__forecast__item">
                <div>
                  <div key={day.day} className="daily__forecast__day">
                    {day.dayOfWeek}
                  </div>
                  <img
                    className="daily__forecast__icon"
                    src={day.weatherCode !== undefined ?  getWeatherIcon(day.weatherCode): ""}
                    alt=""
                  />
                  <div className="daily__forecast__temp">
                    <span className="temp__high">
                      {day.tempMax !== undefined ? formatTemp(
                    ...
                    <span className="temp__low">
                      {day.tempMin !== undefined ? formatTemp(
